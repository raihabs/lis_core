...
<body>
    ...
    <main id="main">
        ...
        <div style="width: 100%; padding: 10px 100px; background: #eee;">
            ...
            <!-- <form method="post" action="order.php" id="form" style="margin-bottom: 100vh;"> -->
            <?php
            session_start();
              $userId = $_SESSION['user_id'];
            $query_code = "SELECT order_code, u_id,d_id, COUNT(*) as name_count FROM `users_orders` 
                WHERE u_id = '" . $userId . "'  GROUP BY order_code
		        ORDER BY `name_count` DESC";
            $items_code = mysqli_query($db, $query_code);
            foreach ($items_code as $code) {
        echo $code['order_code'];
                $sql_product = "SELECT  o_id, order_code, s_id, school_name, u_id, d_id, day_arrival, time_arrival, status, date,COUNT(order_code) FROM `users_orders` WHERE order_code = '" . $code['order_code'] . "' AND u_id = '" . $userId . "' AND status = 'Check Out' OR status = 'In Process' ";
                    ...
                    <div class="bg-white" style="width: 100%; margin: 10px 0; background: #eee; height: auto;">
                        <div class="card bg-white justify-content-between px-5 top">
                            <div class="d-flex">
                                <h5>
                                    ORDER
                                    <span style=" color: #8f00ff;" class="font-weight-bold">#<?php echo $code['order_code']; ?></span>
                                </h5>
                            </div>
                            <!-- <?php
